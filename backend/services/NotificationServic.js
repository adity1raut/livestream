...
class NotificationService {
  ...
  async createNotification(userId, type, message, link = null, fromUserId = null) {
    try {
      ...
      if (fromUserId) {
        try {
          populatedNotification = await notification.populate('fromUser', 'username profile.name profile.profileImage');
        } catch (populateError) {
          console.warn('Failed to populate fromUser, sending notification without user details:', populateError);
  ...
  async sendMessageNotification(receiverUserId, senderUserId, senderUsername, messageContent, conversationId) {
    ...
    await this.createNotification(receiverUserId, 'MESSAGE', message, link, senderUserId);
  }
