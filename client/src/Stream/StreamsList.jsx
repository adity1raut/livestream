...
const StreamsList = () => {
  ...
  const handleEndStream = async (streamId) => {
    ...
    try {
      await axios.put(`/api/stream/${streamId}/end`, {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const filteredStreams = (activeTab === 'live' ? streams : myStreams).filter(stream =>
  ...
  const renderStreamCard = (stream) => (
    <div key={stream._id} className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden hover:border-purple-600 transition-all shadow-lg">
      {/* Thumbnail */}
      ...
      {/* Stream info */}
      <div className="p-4">
        <h3 className="font-semibold text-lg mb-2 text-white line-clamp-2">{stream.title}</h3>
  ...
  const renderStreamList = (stream) => (
    <div key={stream._id} className="bg-gray-800 rounded-xl border border-gray-700 p-4 flex items-center gap-4 hover:border-purple-600 transition-all shadow-lg">
      {/* Thumbnail */}
      ...
      <div className="flex-1">
        <h3 className="font-semibold text-lg mb-1 text-white">{stream.title}</h3>
        ...
        {stream.description && (
          <p className="text-gray-400 text-sm line-clamp-1">{stream.description}</p>
        )}
      </div>
        ...
        {loading ? (
          ...
          <div className={
            ...
            {filteredStreams.map(stream => 
              viewMode === 'grid' ? renderStreamCard(stream) : renderStreamList(stream)
            )}
          </div>
        )}
        ...
        {activeTab === 'my-streams' && myStreams.length > 0 && (
          <div className="mt-8">
            <div className="bg-gray-800 rounded-xl border border-gray-700 shadow-lg p-6">
              <h3 className="text-xl font-semibold text-white mb-4">Stream Statistics</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div className="bg-gray-700 p-4 rounded-lg border border-gray-600">
                  <div className="text-2xl font-bold text-blue-400 mb-1">
                    {myStreams.reduce((acc, s) => acc + (s.viewers?.length || 0), 0)}
                  </div>
